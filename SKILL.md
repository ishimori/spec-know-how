...
### 1.3 機能要件・非機能要件・UI操作モデルの三軸で抽出する

- 個別機能だけを追うと非機能要件が抜ける
- **機能要件（FR: Functional Requirement）**: 「何ができるか」
- **非機能要件（NFR: Non-Functional Requirement）**: ログ/セキュリティ/監査/運用など「どのように機能するか」を独立トラックで管理
- **UI操作モデル（UXM: User eXperience Model）**: Excelライクなグリッド操作等は個別画面仕様と別に管理
...
- 質問による前提確定
- DD への起票指示（**機能要件（FR）/ 非機能要件（NFR）/ UI操作モデル（UXM）**/抽出実行）
- 突合結果に基づく信頼度付与
- QA 向けインデックス化
- 改善課題の収束管理
...
**目的**: 抽出対象、抽出難易度、抽出方式、プロジェクト規模を確定する。
...
上記 9-10 の回答に基づいて、進め方を以下のように分岐させる：
...
- `scale-assessment` — 規模判定結果と進め方の選択

---

### Phase 1: 抽出計画の分割（機能要件・非機能要件・UI操作モデル）

**目的**: 機能偏重を防ぎ、非機能要件を独立管理する。また、規模に応じた段階的実行体制を整える。
...
この DD の成果をベースに Phase 1b で領域別に分割します。
...
- `DD-FR-Domain-A` — ドメイン A の機能要件（FR）
- `DD-NFR-Domain-A` — ドメイン A の非機能要件（NFR）
- `DD-UXM-Domain-A` — ドメイン A の UI操作モデル（UXM）
...
- **【新規】プロジェクト規模（推定 LOC）とドメイン複雑性が判定されている**
- **【新規】進め方の戦略（即座生成 / 標準進行 / 段階的実行 / 全体DD+分割）が決定されている**
- **【新規】生成物の出力先が決定されている**（分析対象ではなく実行元プロジェクト配下）

### Gate 1（抽出計画完了）

- 規模判定の結果に応じて、以下のいずれかが完了している：
  - **小規模（即座生成）**: Gate 0 完了後、Phase 2 以降に直進
  - **標準進行**: 機能要件（FR） DD、非機能要件（NFR） DD、UI操作モデル（UXM） DD が起票されている
  - **段階的実行以上**: 全体構成 DD が起票されている + ドメイン/領域別の分割計画が立案されている
