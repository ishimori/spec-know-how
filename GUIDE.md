...
> **注**: このガイドの Gate 1〜6 は `gates/` フォルダのチェックリストに対応します。`manuals/` 内にも各ステップに Gate がありますが、番号体系が異なります（マニュアルは Step 0〜4 構成。Step 0 は移行判断の前提フェーズ）。

---

## 各 Step の内部手順（共通）

> **【Claude 確認依頼】とは**: 人間が「〜を確認して」と Claude に指示し、Claude が確認・実行する。人間はなるべく Claude に任せる設計。

各 Step には、以下の 5 段階の内部手順がある。②〜⑤は Claude に依頼すること。

```mermaid
flowchart TD
    START([Step N 開始]) --> A
    A["① Step 固有の作業を実施する\n（やること セクションの手順に従う）"]
    A --> B["② 【Claude 確認依頼】\nこのステップで生成した成果物が\nCLAUDE.md または document_map.md に\n索引登録されていなければ追記して"]
    B --> C["③ 【Claude 確認依頼】\nこのステップの成果物で\n機械処理（AST 解析・DDL 突合・Grep 等）により\n信頼度を上げられるものがあれば実行して"]
    C --> D["④ 【Claude 確認依頼】\nDD チケットにしか記載されていない情報が\nあれば仕様書または索引に転記して"]
    D --> E["⑤ 【Claude 確認依頼】\n01_仕様書作成マニュアルの意図に\n沿った作業ができているか確認し\n不足があれば補完して\n（DA 批判レビュー・信頼度ラベル・横断検証の観点で）"]
    E --> GATE(["Gate チェックリストを確認して\n次の Step へ"])
...
> ②〜⑤の確認はすべて Claude に依頼できる。人間の判断が必要なのは、Gate 通過の最終決定のみ。

> **自動化のヒント**: ②〜⑤の手順を CLAUDE.md に常設指示として登録しておけば、毎 Step で Claude に依頼する手間を省ける。ただし書き方やタイミングによっては CLAUDE.md の指示が機能しないことがある。慣れるまでは毎回手動で依頼する方が安全。

> **コンテキストクリア前の確認**: コンテキストをクリアする直前に「コンテキストにしかない情報があるはずです。ドキュメントに出力してください」と Claude に伝えると、ドキュメントへの転記漏れを拾えることがある。
