...
- **GUIDE_IMPL.md**: 実装のクイックリファレンス（manuals/02 の要約版、5ステップ + mermaid フローチャート）
  - GUIDE.md（仕様抽出）と対になる構成
  - 垂直スライス → 核機能 → 放射的実装 → 品質ゲート → 拡張・デプロイ
- **README.md**: 「なぜこのリポジトリが必要か」セクション追加
  - LLM 活用時のペインポイント（仕様不在・業務ルール見落とし・検証基準なし・属人的進行）を明示

#### 変更

- **README.md**: クイックリファレンスとマニュアルの役割分担を2x2テーブルに再構成
