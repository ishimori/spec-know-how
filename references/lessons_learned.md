...
> **出典**: `housing-e-kintai-next`（Streamlit → React+FastAPI 移行）プロジェクトでの実践記録。
> 固有のDD番号やファイル名が登場するが、パターンは他のリエンジニアリング案件にも汎用的に適用できる。
>
...
## リエンジニアリング調査フェーズで得られた気付き（jp-kvmanage プロジェクト・DD-036〜037）

> **出典**: `jp-kvmanage`（CakePHP 3.7 → 新システム移植調査）プロジェクト。
> Gate3（画面・UI）通過後に 15本の後追い DD が発生した原因を分析した結果。
>
> **反映DD**: DD-036〜037（jp-kvmanage-new）/ sample_prompts 00〜04 修正

| セクション | 対象DD | 追記日 |
|-----------|--------|--------|
| 調査フェーズ気付き (#16〜#18) | DD-036〜037 | 2026-02-20 |

### 16. Gate0 で「調査支援スクリプト」を作っておかないと、後続 Gate で推測頼みになる

リエンジニアリング調査では、LLM がコードを読んで「このフラグは 0/1 しかない」「このテーブルは n 件のはず」と推測する場面が多い。しかし実データと乖離することがある。

jp-kvmanage では以下の乖離が Gate3 通過後に後追い DD として発覚した:

| 後追い DD | 内容 | Gate0 スクリプトがあれば |
|-----------|------|----------------------|
| DD-025 | `lost_flag` に値 2 が存在（コードからは 0/1 のみ推測） | `flag_distribution.py` で即発見 |
| DD-026 | `concurrently_employees` テーブルの実存在が不明 | `db_objects.py` で即確認 |
| DD-027 | `key_masters` / `key_owner_histories` の件数不一致 | `table_stats.py` で即発見 |

**教訓**: Gate0（初期設定）の時点で、後続 Gate で使う DB 解析スクリプト（Python）を用意しておけ。
`flag_distribution.py`・`table_stats.py`・`db_objects.py` の 3本があれば、Gate2〜4 で推測を実測に置き換えられる。
「Gate2 で実DBアクセスしよう」と書くだけでは不十分。ツールがなければ実行できない。

### 17. 認可マトリクスを Gate3 で「旧システムの事実」として記録しないと後で設計と混同される

Gate3（画面・UI）で権限確認を「○○権限で表示制御あり」と記録するだけでは不十分。
ロール × 画面 の**マトリクス表**として記録しないと、後から「どのロールが何にアクセスできるか」を再調査するDDが必要になる（jp-kvmanage では DD-021 が後追いで発生）。

また、旧システムに「兼務ユーザー（複数ロール保持）」がいる場合、その画面アクセス挙動を Gate3 で確認しないと、後から専用 DD が 2〜3本必要になる（jp-kvmanage では DD-022/023 が後追いで発生）。

**教訓**: Gate3 の権限確認では「認可マトリクスを旧システムの事実として記録する」を必須にせよ。
記録する際は「旧システムの調査」であって「新システムの設計」ではないことを明示すること（混同すると doc/context の書き分けが崩れる）。

### 18. フラグ・ステータスの「書き込みコードパス」を Gate4 で追わないと、設計バグが仕様書に見えない

Gate4（業務ロジック抽出）で状態遷移を「仕様書に記述した」だけでは不十分。
各状態への遷移を**どの関数がトリガーするか**を確認しないと、「更新すべきだが更新しない設計バグ」が仕様書上で見えない。

jp-kvmanage では `lost()` / `lostCancel()` が `key_owner_histories` を一切更新しない設計バグにより、126件のデータ不整合が発生していた。このバグは Gate4 で「コードパスを追う」観点がなかったため、DD-025（Gate3 後追い）と DD-030（Gate4 で改めて調査）の 2本に分かれて発覚した。
