...
| 作成日 | 更新日 | ステータス |
|--------|--------|------------|
| 2026-02-24 | 2026-02-24 | 待機中（DD-002〜007 完了待ち） |
...
- [x] SP-000 で Claude が聞くべき質問項目を定義する
- [x] SP-000 の記録形式（構造化 Markdown）を設計する
- [x] SP-000 テンプレートを作成する（→ [doc/SP/SP-000.md](doc/SP/SP-000.md)）
- [ ] 動作確認（Claude が質問→ユーザー回答→記録 のフローが機能するか）※実プロジェクトで初使用時に確認
...
- [ ] **子チケット動的追加操作を実装する**（↓ 申し送り参照）
- [ ] 😈 **DA 批判レビュー（「このPhaseで何が壊れるか」を最低1件発見）**

#### ⚠️ 申し送り: 子チケット動的追加の設計要件

SP-001（および将来の SP-002〜006）では、調査中に追加タスクが発覚した場合に
**子チケットを動的に追加**する操作が必要。現状は SP-001.md の Step 5 に
「SP-001-04.md 等を新規作成」と書いてあるだけで、Claude の推測に依存している（脆弱）。

SP スキルでエンコードすべき動作：

| 動作 | 仕様 |
|------|------|
| 番号の決め方 | context と gate を除く既存子チケットの最大番号 + 1（例: 01〜03 なら 04） |
| テンプレート | SP-001-01.md をベースに作業セクションのみ書き換え |
| 子チケット一覧の更新 | SP-001.md の「子チケット一覧」テーブルに追記（context/gate の前の行に挿入） |
| context/gate の実行順 | 動的追加チケットが全て完了してから context → gate を実行する |
...
**DA 観点:** SP-000 テンプレートが Claude の指示として機能するか？

| # | 発見した問題/改善点 | 重要度 | 再現手順 | DA 観点 | 対応 |
|---|-------------------|--------|----------|--------|------|
| 1 | 「SP への引き継ぎメモ」を Claude が自動で埋めない可能性がある | 中 | SP-000 完了後に SP-001 を開いた Claude が引き継ぎメモを参照せず動く | 指示の網羅性 | ✅ SP-001〜006 のテンプレートに「SP-000 の引き継ぎメモを必ず確認すること」と明記する |
| 2 | 「わからない」と回答された項目を Claude が後で上書きしない可能性 | 低 | SP-002 開始時に「DBあり」と判明しても SP-000 の「DB 有無: わからない」が更新されない | 記録の鮮度 | ✅ 各 SP テンプレートに「SP-000 の該当項目を確認し、判明した場合は更新すること」を追記する |
